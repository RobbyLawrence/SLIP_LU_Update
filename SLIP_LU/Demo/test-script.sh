...
# Write the header to our CSV file
echo "x,speedup,nnz_change" > "$DATA_FILE"
...
for (( x=1; x<=UPPER_BOUND; x++ )); do
    ...
    # If both values were successfully parsed, log them
    if [[ -n "$speedup" && -n "$nnz_change" ]]; then
        echo "$x,$speedup,$nnz_change" >> "$DATA_FILE"
    else
...
print()
print("=== SPARSITY STATISTICS (factor nonzero change) ===")
